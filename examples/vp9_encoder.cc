...
int main() {
    // Configure encoder for VP9
    media::VideoEncoderConfig config;
    config.output_codec = media::CodecType::VP9;
    config.width = 1920;
    config.height = 1080;
    config.framerate = 30;
    config.bitrate = 5000000;

    // Set VP9-specific parameters
    media::codec::VP9Params vp9_params;
    vp9_params.quality = 23;             // Quality setting (0-63, lower is better)
    vp9_params.speed = "realtime";           // Options: realtime, good, best
    vp9_params.profile = "0";            // Options: 0, 1, 2, 3
    vp9_params.keyframe_interval = 120;
    vp9_params.constant_bitrate = false;
    vp9_params.threads = 0;              // 0 = auto
    vp9_params.tile_columns = 0;         // Log2 of number of tile columns (0-6)
    vp9_params.tile_rows = 0;            // Log2 of number of tile rows (0-2)
    config.SetVP9Params(vp9_params);

    // Create the VP9 encoder instance using the factory method
    auto encoder = media::VideoEncoder::Create(config);
    if (!encoder) {
        std::cerr << "Failed to create VP9 encoder." << std::endl;
        return -1;
    }
