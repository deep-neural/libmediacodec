...
std::unique_ptr<NvidiaH264Encoder> NvidiaH264Encoder::Create(
    const NvidiaH264EncoderConfig& config) {
  std::unique_ptr<NvidiaH264Encoder> encoder(new NvidiaH264Encoder(config));
...
NvidiaH264Encoder::NvidiaH264Encoder(const NvidiaH264EncoderConfig& config)
    : width_(config.width),
...
NvidiaH264Encoder::~NvidiaH264Encoder() {
  if (packet_) {
...
bool NvidiaH264Encoder::Initialize(const NvidiaH264EncoderConfig& config) {
  // Find the h264_nvenc encoder
  const AVCodec* codec = avcodec_find_encoder_by_name("h264_nvenc");
  if (!codec) {
    std::cerr << "NVIDIA H264 encoder not found" << std::endl;
    return false;
  }
...
bool NvidiaH264Encoder::EncodeYUV420(const std::vector<uint8_t>& yuv_data,
...
bool NvidiaH264Encoder::EncodeNV12(const std::vector<uint8_t>& nv12_data,
                                   std::vector<uint8_t>* encoded_frame) {
...
bool NvidiaH264Encoder::EncodeFrame(const std::vector<uint8_t>& frame_data,
                                   std::vector<uint8_t>* encoded_frame,
                                   bool is_nv12) {
  // Clear output buffer
  encoded_frame->clear();
